...
const CITY_COORDS = (() => {
  ...
  for (const p of MOCK_PLACES) {
    if (p.city && !m.has(p.city.toLowerCase())) m.set(p.city.toLowerCase(), [p.lat, p.lng]);
  }
  return m;
})();

function coordsFor(id: string, country: string, city?: string | null): [number, number] | null {
  const base = (city && CITY_COORDS.get(city.trim().toLowerCase()))
    ?? COUNTRY_BY_NAME.get(country.trim().toLowerCase())?.center
    ?? null;
  if (!base) return null;
  const h = hash(id);
...
export async function fetchLocatedTravelers(): Promise<LocatedTraveler[]> {
  ...
  const [tripsRes, profilesRes] = await Promise.all([
    supabase
      .from("travel_intents")
      .select("user_id, destination_country, destination_city, starts_on, ends_on, budget_eur, languages, interests, bio")
