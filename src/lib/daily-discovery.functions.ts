...
function clean(value: unknown, max: number) {
  return (
    String(value ?? "")
      .normalize("NFKC")
      // eslint-disable-next-line no-control-regex -- non-printable catalog input is intentionally removed
      .replace(/[\u0000-\u001F\u007F]/g, " ")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, max)
...
function sanitizeGeneratedItems(value: unknown, sources: SearchResult[]): DailyDiscoveryItem[] {
  ...
  return value
    .flatMap((raw) => {
      ...
      if (!allowedKinds.has(kind) || !title || !summary || !source || seen.has(title.toLowerCase()))
        return [];
...
async function buildSnapshot(): Promise<DailyDiscovery | null> {
  ...
  await db.rpc("cleanup_old_daily_discovery_snapshots").then(
    () => undefined,
    () => undefined,
  );
  ...
  if (!existing) {
    ...
    if (error) {
      ...
      if (concurrent?.status === "ready")
        return {
          date,
          generatedAt: concurrent.generated_at,
          isLive: true,
          items: concurrent.payload?.items ?? [],
        };
      return null;
    }
    ...
    await db
      .from("daily_discovery_snapshots")
      .update({ generated_at: generatedAt, status: "building" })
      .eq("snapshot_date", date);
  } else {
    return null;
  }
