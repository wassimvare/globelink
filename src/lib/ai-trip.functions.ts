...
const ALLOWED_STYLES = new Set(["Équilibré", "Aventure", "Culture", "Chill", "Foodie", "Nature", "Luxe", "Backpacker", "Romantique", "Famille"]);
const ALLOWED_INTERESTS = new Set(["Plages", "Randonnée", "Street food", "Musées", "Vie nocturne", "Surf", "Yoga", "Photo", "Design", "Vin & gastronomie", "Îles", "Temples"]);
...
export const generateTripPlan = createServerFn({ method: "POST" })
  .inputValidator((data: unknown): Input => {
    ...
    if (!Number.isFinite(days) || days < 1 || days > 60) throw new Error("La durée doit être comprise entre 1 et 60 jours.");
    if (!Number.isFinite(budget) || budget < 100 || budget > 100_000) throw new Error("Le budget doit être compris entre 100 € et 100 000 €.");
    if (!Number.isFinite(travelers) || travelers < 1 || travelers > 20) throw new Error("Le nombre de voyageurs doit être compris entre 1 et 20.");
  ...
  .handler(async ({ data }) => {
    const { provider, modelId } = createTravelAiProvider();
