...
export function QuickCreate({ compact = false, className }: { compact?: boolean; className?: string }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="quick-create-sheet max-h-[82dvh] overflow-hidden rounded-t-[1.75rem] border-border/70 bg-card px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-5 shadow-elevated sm:left-auto sm:right-4 sm:top-20 sm:h-auto sm:max-h-[calc(100vh-6rem)] sm:w-[420px] sm:rounded-2xl sm:border sm:p-6">
        <SheetHeader className="pr-10 text-left">
          ...
          <SheetTitle className="font-display text-xl sm:text-2xl">Qu’est-ce que tu veux faire ?</SheetTitle>
          <SheetDescription className="text-xs leading-relaxed sm:text-sm">Choisis une action sans quitter ta page actuelle.</SheetDescription>
        </SheetHeader>
        ...
        <div className="quick-create-list mt-4 grid max-h-[calc(82dvh-10rem)] gap-2 overflow-y-auto overscroll-contain pr-1 sm:mt-6 sm:max-h-none sm:gap-3 sm:overflow-visible sm:pr-0">
          {actions.map((action) => {
            ...
            return (
              ...
              >
                <div className={cn("grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br sm:h-12 sm:w-12", action.accent)}>
                  <Icon className="h-5 w-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold text-foreground sm:text-base">{action.title}</div>
                  <div className="mt-0.5 text-xs leading-snug text-muted-foreground sm:text-sm">{action.description}</div>
                </div>
                <span className="text-lg text-muted-foreground transition group-hover:translate-x-1 group-hover:text-primary">→</span>
              </Link>
            );
          })}
        </div>
