...
type Liker = { user_id: string; username: string; display_name: string | null; avatar: string | null };
...
export function StoryLikeBar({ storyId, ownerId }: { storyId: string; ownerId?: string }) {
  ...
  const toggle = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["story-likes", storyId, user?.id] }),
      ...
      >
        {!isOwner && (
          <button
            ...
            className="grid h-12 w-12 place-items-center rounded-full bg-white/15 text-white backdrop-blur transition active:scale-90"
          >
      ...
      {isOwner && showList && (
        ...
        >
          ...
          >
            ...
            {count === 0 && (
              <p className="py-6 text-center text-sm text-muted-foreground">Personne n'a encore aimé cette story.</p>
            )}
