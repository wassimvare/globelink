...
export function BottomNav() {
  ...
  const items: Array<{ to: string; label: string; Icon: typeof Home; exact?: boolean; requiresAuth?: boolean }> = [
  ...
  return (
    <nav aria-label="Navigation principale" className="mobile-bottom-nav fixed inset-x-0 bottom-0 z-40 sm:hidden">
      <div className="mobile-bottom-nav-inner grid grid-cols-[1fr_1fr_56px_1fr_1fr] items-center border-t border-border/70 bg-card/96 px-1.5 pt-1 shadow-[0_-10px_35px_-24px_rgba(3,28,43,.45)] backdrop-blur-xl">
        ...
        <div className="relative -mt-4 flex justify-center"><QuickCreate compact /></div>
...
function NavItem({ to, label, Icon, exact }: { to: string; label: string; Icon: typeof Home; exact?: boolean }) {
  return (
    <Link
      to={to as any}
