...
export function PostCard({ post }: { post: Post }) {
  ...
  const media: Media[] = post.post_media?.length
    ...
    : post.video_url
      ? [{ id: "legacy-video", url: post.video_url, media_type: "video", position: 0 }]
  ...
  const applyReaction = useMutation({
    ...
    onError: (e: Error) => {
      if (e.message === "auth") {
        ...
        navigate({ to: "/auth", search: { redirect: `/post/${post.id}` } });
  ...
  const toggleLike = useMutation({
    ...
    onError: (e: Error) => {
      if (e.message === "auth") {
        ...
        navigate({ to: "/auth", search: { redirect: `/post/${post.id}` } });
      } else toast.error("Like non enregistré. Réessaie.");
    },
  });
  ...
  const toggleSave = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["post-saved", post.id] });
      toast.success(saved ? "Retiré des favoris" : "Ajouté aux favoris");
    },
    onError: (e: Error) => {
      if (e.message === "auth") {
        toast.info("Crée un compte pour enregistrer cette publication.");
        navigate({ to: "/auth", search: { redirect: `/post/${post.id}` } });
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share)
        await navigator.share({ title: "GlobeLink", text: post.caption ?? "", url });
      else {
  ...
  const doFollow = useMutation({
    ...
    onError: (e: Error) => {
      if (e.message === "auth") {
        toast.info("Crée un compte pour suivre ce voyageur.");
        navigate({ to: "/auth", search: { redirect: `/post/${post.id}` } });
    ...
    return (
      <div className="surface-subtle flex items-center justify-between gap-4 rounded-3xl p-5 text-sm text-muted-foreground">
        <span className="inline-flex items-center gap-2">
          <EyeOff className="h-4 w-4" /> Publication masquée de ton fil.
        </span>
        <button
          type="button"
          onClick={() => setHidden(false)}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 font-semibold text-foreground transition hover:shadow-soft"
        >
    ...
    <article className="group surface-card interactive-card media-polish animate-rise overflow-hidden rounded-[1.75rem]">
      <header className="flex items-center gap-3 p-4">
        <Link
          to="/profile/$username"
          params={{ username }}
          className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-secondary shrink-0 ring-2 ring-transparent transition hover:ring-accent"
        >
          {avatarUrl ? (
            <img
              src={avatarUrl}
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="rounded-2xl border-border/70 bg-card/95 p-2 shadow-elevated backdrop-blur-2xl"
            >
      ...
      <div
        className={`relative overflow-hidden bg-muted ${isReel ? "aspect-[9/16] max-h-[80vh] mx-auto w-full" : "aspect-square"}`}
      >
        {(() => {
          if (!mediaUrl) {
              ...
              return (
                <div className="relative h-full w-full bg-black">
                  ...
                  <div className="absolute inset-0 grid place-items-center bg-black/20">
                    <span className="inline-flex items-center gap-2 rounded-full bg-black/60 px-3 py-2 text-xs font-medium text-white backdrop-blur">
              ...
              <video
                ...
                onEnded={() => {
                  ...
                  if (
                    !isReel &&
                    next &&
                    (next.media_type === "video" || next.media_type === "reel")
                  )
          ...
          <>
            {idx > 0 && (
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 grid h-9 w-9 place-items-center rounded-full glass shadow-soft"
              >
            ...
            {idx < media.length - 1 && (
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 grid h-9 w-9 place-items-center rounded-full glass shadow-soft"
              >
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-center gap-4">
          ...
          >
            <Heart
              className={`h-6 w-6 transition ${liked ? "fill-destructive text-destructive scale-110" : ""}`}
            />
            <span className="font-medium">{likeCount}</span>
          </button>
          <div className="group/react relative">
            <button
              className="flex items-center gap-1 rounded-full px-2 py-1 text-sm transition hover:bg-secondary"
              aria-label="Réagir"
            >
              {myReaction ? (
            ...
            <div className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 flex -translate-x-1/2 gap-1 rounded-full border border-border bg-card p-1.5 opacity-0 shadow-elevated transition group-hover/react:pointer-events-auto group-hover/react:opacity-100">
              {REACTIONS.map((r) => (
                <button
                  key={r.key}
                  onClick={() => applyReaction.mutate(myReaction === r.key ? null : r.key)}
          ...
          <button
            onClick={share}
            className="ml-auto grid h-9 w-9 place-items-center rounded-full transition hover:bg-secondary"
          >
            <Share2 className="h-5 w-5" />
          </button>
          <button
            onClick={() => toggleSave.mutate()}
            className="grid h-9 w-9 place-items-center rounded-full transition hover:bg-secondary active:scale-90"
          >
        ...
        {post.hashtags && post.hashtags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {post.hashtags.map((h) => (
              <span key={h} className="text-xs font-medium text-accent">
                #{h}
              </span>
            ))}
          </div>
