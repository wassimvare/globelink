...
export function CallProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const update = () => setDurationSeconds(Math.max(0, Math.floor((Date.now() - active.connectedAt!) / 1000)));
    update();
  ...
  const insertRtcMessage = useCallback(async (
    session: CallSession,
    signal: RtcSignal,
    payload?: unknown,
  ) => {
    ...
    if (error) console.error("RTC signal error", error);
  }, [user]);

  const insertRawRtcMessage = useCallback(async (
    conversationId: string,
    recipientId: string,
    callId: string,
    kind: CallKind,
  ...
  const cleanupRtcMessages = useCallback(async (callId: string) => {
    if (!user) return;
  ...
  const logCall = useCallback(async (session: CallSession, reason: CallEndReason) => {
    if (!user || session.direction !== "outgoing" || loggedCallsRef.current.has(session.callId)) return;
    loggedCallsRef.current.add(session.callId);
    const seconds = session.connectedAt ? Math.max(0, Math.floor((Date.now() - session.connectedAt) / 1000)) : 0;
  ...
  const resetCall = useCallback((session: CallSession | null, reason: CallEndReason, shouldLog = true) => {
    clearTimeouts();
    stopMedia();
    setIncoming(null);
    setActive(null);
  ...
  const sendHangupAndReset = useCallback(async (reason: CallEndReason = "ended") => {
  ...
  const sendAnswer = useCallback(async (session: CallSession) => {
    const pc = peerConnectionRef.current;
  ...
  const applyOffer = useCallback(async (offer: RTCSessionDescriptionInit) => {
    const pc = peerConnectionRef.current;
  ...
  const createPeerConnection = useCallback((session: CallSession, stream: MediaStream) => {
    peerConnectionRef.current?.close();
    ...
    pc.onconnectionstatechange = () => {
      if (pc.connectionState === "connected") {
        setActive((current) => {
          ...
          const next = { ...current, phase: "connected" as const, connectedAt: current.connectedAt ?? Date.now() };
  ...
  const acquireMedia = useCallback(async (kind: CallKind, facing: "user" | "environment" = "user") => {
    if (!mediaIsAvailable()) {
  ...
  const processSignal = useCallback(async (
    ...
    if (!meta.call_id || !meta.signal || !meta.recipient_id || meta.recipient_id !== user.id) return;
    ...
    const session = current?.callId === callId ? current : ringing?.callId === callId ? ringing : null;
    if (!session) return;

    if (meta.signal === "offer") {
      const offer = meta.payload as RTCSessionDescriptionInit;
      if (peerConnectionRef.current && activeRef.current?.callId === callId) await applyOffer(offer);
    ...
    if (meta.signal === "ice") {
      ...
      if (pc?.remoteDescription) {
        try { await pc.addIceCandidate(candidate); } catch (error) { console.warn("ICE candidate ignored", error); }
      } else {
  ...
  }, [applyOffer, clearTimeouts, flushCandidates, insertRawRtcMessage, resetCall, user]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`globelink-calls-${user.id}`)
    ...
    supabase
      ...
      .limit(250)
      .then(({ data }: { data: RtcMessageRow[] | null }) => {
  ...
  const startCall = useCallback(async (args: StartCallArgs) => {
    if (!user) return;
    ...
    if (!mediaIsAvailable()) {
      toast.error("Les appels nécessitent l'adresse HTTPS. Lance le fichier GLOBELINK_APPELS_HTTPS.bat.", { duration: 8000 });
    ...
    const session: CallSession = {
      ...
      direction: "outgoing",
      phase: "calling",
      peer: { id: args.recipientId, name: args.recipientName, avatar: args.recipientAvatar ?? null },
      createdAt: Date.now(),
      connectedAt: null,
    };
    ...
    try {
      ...
      const offer = await pc.createOffer({ offerToReceiveAudio: true, offerToReceiveVideo: args.kind === "video" });
      await pc.setLocalDescription(offer);
      await insertRtcMessage(session, "offer", offer);
      outgoingTimeoutRef.current = window.setTimeout(async () => {
        if (activeRef.current?.callId !== session.callId || activeRef.current.phase === "connected") return;
  ...
  }, [acquireMedia, createPeerConnection, insertRtcMessage, resetCall, user]);
  ...
  const acceptCall = useCallback(async () => {
    ...
    clearTimeouts();
    if (!mediaIsAvailable()) {
      toast.error("Ouvre GlobeLink avec l'adresse HTTPS pour utiliser les appels", { duration: 8000 });
  ...
  }, [acquireMedia, applyOffer, clearTimeouts, createPeerConnection, insertRtcMessage, resetCall]);
...
function Avatar({ profile, large = false }: { profile: PeerProfile; large?: boolean }) {
  ...
  return profile.avatar ? (
    <img src={profile.avatar} alt="" className={`${size} rounded-full border-4 border-white/15 object-cover shadow-2xl`} />
  ) : (
    <div className={`${size} grid place-items-center rounded-full border-4 border-white/15 bg-white/10 font-semibold text-white shadow-2xl`}>
...
function IncomingCallScreen({
  ...
  return (
    <div className="call-screen call-screen-incoming" role="dialog" aria-modal="true" aria-label={`${callLabel(session.kind)} entrant`}>
      ...
      <div className="relative z-10 flex min-h-[100dvh] flex-col items-center justify-between px-6 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(3rem,env(safe-area-inset-top))] text-center text-white">
        <div>
          <div className="mx-auto mb-5 grid h-12 w-12 place-items-center rounded-full bg-white/10 backdrop-blur">
            {session.kind === "video" ? <Video className="h-5 w-5" /> : <PhoneCall className="h-5 w-5" />}
          </div>
          <p className="text-sm font-medium uppercase tracking-[0.22em] text-white/65">{callLabel(session.kind)} entrant</p>
        </div>
        <div className="flex flex-col items-center">
          <div className="call-avatar-pulse"><Avatar profile={session.peer} large /></div>
        ...
        <div className="grid w-full max-w-xs grid-cols-2 gap-8">
          <CallActionButton label="Refuser" tone="danger" onClick={onDecline} icon={<PhoneOff className="h-7 w-7" />} />
          <CallActionButton label="Accepter" tone="success" onClick={onAccept} icon={session.kind === "video" ? <Video className="h-7 w-7" /> : <Phone className="h-7 w-7" />} />
        </div>
      </div>
    </div>
  );
...
function ActiveCallScreen({
  ...
  const status = session.phase === "calling"
    ? "Appel en cours…"
  ...
  return (
    <div className={`call-screen ${session.kind === "video" ? "call-screen-video" : "call-screen-audio"}`} role="dialog" aria-modal="true" aria-label={callLabel(session.kind)}>
      {session.kind === "video" ? (
        <>
          <video ref={remoteVideo} autoPlay playsInline muted={!remoteSound} className={`call-remote-video ${remoteStream ? "opacity-100" : "opacity-0"}`} />
          {!remoteStream && (
            <div className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_50%_30%,#155e75,#071923_68%)]">
              <div className="flex flex-col items-center text-white"><Avatar profile={session.peer} large /><p className="mt-6 text-white/70">{status}</p></div>
            </div>
          )}
      ...
      <div className="call-topbar">
        ...
        {!secureContextBadgeHidden() && (
          <div className="flex items-center gap-1 rounded-full bg-amber-400/15 px-3 py-1.5 text-xs text-amber-100"><ShieldAlert className="h-3.5 w-3.5" /> HTTPS</div>
        )}
      </div>

      <div className="call-controls">
        <CallControl label={muted ? "Réactiver" : "Micro"} active={!muted} onClick={onToggleMute} icon={muted ? <MicOff /> : <Mic />} />
        {session.kind === "video" && <CallControl label={cameraEnabled ? "Caméra" : "Activer"} active={cameraEnabled} onClick={onToggleCamera} icon={cameraEnabled ? <Camera /> : <CameraOff />} />}
        <button type="button" onClick={onHangup} className="call-hangup" aria-label="Raccrocher"><PhoneOff className="h-7 w-7" /></button>
        <CallControl label={remoteSound ? "Son" : "Silencieux"} active={remoteSound} onClick={onToggleSound} icon={remoteSound ? <Volume2 /> : <VolumeX />} />
        {session.kind === "video" && <CallControl label="Retourner" active onClick={onSwitchCamera} icon={<RefreshCw />} />}
      </div>
    </div>
  );
}
...
function CallActionButton({
  ...
  return (
    <button type="button" onClick={onClick} className="flex flex-col items-center gap-3 text-sm text-white/80">
      <span className={`grid h-17 w-17 place-items-center rounded-full shadow-2xl ${tone === "danger" ? "bg-red-500" : "bg-emerald-500"}`}>{icon}</span>
      {label}
    </button>
  );
}
