  ...
  head: () => ({
    meta: [
      { title: "Assistant voyage — GlobeLink" },
      {
        name: "description",
        content:
          "Crée un itinéraire de voyage personnalisé en quelques secondes avec l'assistant GlobeLink.",
      },
    ],
  }),
...
function AiTripPage() {
  ...
  const plan = useMutation({
    mutationFn: async () => {
      ...
      if (!user) throw new Error("Connecte-toi pour utiliser l'assistant voyage.");
      ...
      <main className="page-container pb-16">
        <header className="page-heading">
          <div className="max-w-2xl">
            <div className="eyebrow">
              <Sparkles className="h-4 w-4" /> Conseiller voyage
            </div>
            <h1 className="mt-2 font-display text-3xl font-semibold sm:text-5xl">
              Prépare un voyage qui te ressemble.
            </h1>
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground sm:text-base">
              Indique l’essentiel et GlobeLink te propose un programme clair, réaliste et adapté au
              budget du groupe.
            </p>
          </div>
          <div className="hidden items-center gap-2 rounded-2xl border border-border/70 bg-card/70 p-3 text-xs text-muted-foreground shadow-soft backdrop-blur lg:flex">
            ...
            <div>
              <strong className="block text-foreground">Conseils personnalisés</strong>Le programme
              est adapté à la durée, au budget et aux centres d’intérêt indiqués.
            </div>
          </div>
        </header>
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <section className="surface-card rounded-[2rem] p-4 sm:p-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Destination" hint="Pays, ville ou région">
                <Input
                  ...
                  placeholder="Japon, Bali, Portugal…"
                />
              </Field>
              <Field
                label="Durée"
              ...
              >
                <Input
                  ...
                  max={60}
                  value={days}
                  onChange={(e) => setDays(clamp(Number(e.target.value), 1, 60))}
                />
              </Field>
              <Field
                label="Budget total"
                hint="Transport compris ou non, selon ton choix"
              ...
              >
                <div className="relative">
                  <Input
                    ...
                    step={50}
                    value={budget}
                    onChange={(e) => setBudget(clamp(Number(e.target.value), 100, 100000))}
                  />
              ...
              >
                <Input
                  ...
                  max={20}
                  value={travelers}
                  onChange={(e) => setTravelers(clamp(Number(e.target.value), 1, 20))}
                />
              </Field>
            </div>
              ...
              {STYLES.map((item) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`pressable inline-flex min-h-10 items-center gap-1.5 rounded-full border px-3.5 py-2 text-sm font-semibold ${active ? "border-primary bg-primary text-primary-foreground shadow-soft" : "border-border/80 bg-secondary/65 text-foreground/75 hover:border-primary/35 hover:bg-card"}`}
                  >
            ...
            <div className="mt-6">
              <ChoiceGroup
                ...
                subtitle="Jusqu'à 8 choix pour affiner le programme."
                action={
                  interests.length > 0 ? (
                    <button
                      type="button"
                ...
                {INTEREST_TAGS.map((tag) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={active}
                      className={`pressable min-h-10 rounded-full border px-3.5 py-2 text-sm ${active ? "border-accent bg-accent text-accent-foreground shadow-soft" : "border-border/80 bg-secondary/65 text-foreground/75 hover:border-accent/40 hover:bg-card"}`}
                    >
                      {tag}
                    </button>
                  );
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Button
                onClick={() => plan.mutate()}
                disabled={!isValid || plan.isPending}
                size="lg"
                className="h-12 flex-1 rounded-2xl bg-primary text-primary-foreground shadow-soft transition hover:-translate-y-0.5 hover:shadow-glow active:translate-y-0"
              >
                {plan.isPending ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Préparation de l’itinéraire…
                  </>
                ) : (
                  <>
                    <Wand2 className="mr-2 h-4 w-4" /> Préparer mon voyage
                  </>
                )}
              </Button>
              <Button
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2 text-xs">
              ...
              {EXAMPLES.map((item) => (
                <button
                  ...
                  className="pressable rounded-full border border-border/70 bg-background/70 px-3 py-1.5 font-medium text-muted-foreground hover:border-primary/30 hover:text-foreground"
                >
          ...
          <aside className="surface-card rounded-[2rem] p-5 lg:sticky lg:top-20">
            ...
            <div className="mt-5 space-y-3">
              ...
              <SummaryLine
                label="Groupe"
                value={`${travelers} voyageur${travelers > 1 ? "s" : ""}`}
              />
            ...
            <div className="rounded-2xl bg-primary/[0.06] p-4">
              ...
              <div className="mt-2 font-display text-3xl font-semibold">
                {budgetPerPerson.toLocaleString("fr-FR")} €
              </div>
              <div className="text-sm text-muted-foreground">
                par personne · environ {budgetPerDay.toLocaleString("fr-FR")} € / jour
              </div>
            </div>
            <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
              L’itinéraire est une aide à la préparation. Vérifie les horaires, tarifs, formalités
              et disponibilités avant de réserver.
            </p>
            <Button asChild variant="outline" className="mt-4 w-full rounded-xl">
              <Link to="/ai-pro">
                <Sparkles className="mr-2 h-4 w-4" /> Poser une question au conseiller
              </Link>
            </Button>
          </aside>
        </div>
        ...
        {plan.data && (
          <article className="surface-card mt-8 animate-rise overflow-hidden rounded-[2rem] p-5 sm:p-8">
            <div className="flex flex-col gap-4 border-b border-border/70 pb-5 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="eyebrow">Ton itinéraire</div>
                <h2 className="mt-1 font-display text-3xl sm:text-4xl">
                  {destination} en {days} jours
                </h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  {travelers} voyageur(s) · {budget.toLocaleString("fr-FR")} € · style {style}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(plan.data!.plan);
                    toast.success("Itinéraire copié");
                  }}
                >
                  <Download className="mr-2 h-4 w-4" /> Copier
                </Button>
                <Button size="sm" className="rounded-full" onClick={savePlan}>
                  <Save className="mr-2 h-4 w-4" /> Enregistrer
                </Button>
              </div>
            </div>
            <div className="md-body mt-6">
              <ReactMarkdown>{plan.data.plan}</ReactMarkdown>
            </div>
          </article>
        )}
        ...
        {!plan.data && !plan.isPending && (
          <div className="surface-subtle mt-8 rounded-[2rem] p-8 text-center text-sm text-muted-foreground">
            ...
            Ton itinéraire apparaîtra ici avec le programme jour par jour, le budget, les
            hébergements et la check-list.
          </div>
        )}

        <div className="mt-7 text-center text-xs text-muted-foreground">
          <Link to="/trips" className="font-semibold underline underline-offset-4">
            Retrouver mes voyages sauvegardés
          </Link>
        </div>
      </main>
    </div>
