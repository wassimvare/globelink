...
export const Route = createFileRoute("/_authenticated/settings/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Personnalise ta photo, ton pseudo, ta bio et tes réseaux sociaux sur GlobeLink." },
...
const schema = z.object({
  username: z.string().trim().min(3, "3 caractères minimum").max(30, "30 caractères maximum")
...
const toList = (v: string) => v.split(",").map((s) => s.trim()).filter(Boolean).slice(0, 20);
...
function EditProfilePage() {
  ...
  const [form, setForm] = useState({
    username: "", display_name: "", bio: "", country: "", city: "", birth_date: "",
    travel_style: "", website_url: "", instagram: "", tiktok: "", youtube: "", x_handle: "",
    languages: "", interests: "", visited_countries: "",
  });
  ...
  useEffect(() => { getSignedMediaUrl(avatarPath).then(setAvatarUrl); }, [avatarPath]);
  useEffect(() => { getSignedMediaUrl(bannerPath).then(setBannerUrl); }, [bannerPath]);
  ...
  const pickImage = async (kind: "avatar" | "banner", file: File | undefined) => {
    ...
    try {
      ...
      if (kind === "avatar") setAvatarPath(path); else setBannerPath(path);
      toast.success(kind === "avatar" ? "Photo prête — pense à enregistrer" : "Bannière prête — pense à enregistrer");
    } catch {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") { setErrors({ username: "Ce pseudo est déjà pris" }); toast.error("Ce pseudo est déjà pris"); return; }
        throw error;
      }
  ...
  if (isLoading) {
    return (
      <div className="app-page">
        <AppHeader />
        <div className="mx-auto max-w-2xl px-4 py-16 text-center text-muted-foreground">Chargement…</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><h1 className="font-display text-3xl">Modifier mon profil</h1><p className="mt-1 text-sm text-muted-foreground">Photo, pseudo, biographie, infos et réseaux sociaux.</p></div>
          <Link to="/security" className="pressable inline-flex min-h-11 items-center justify-between gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.07] px-4 text-sm font-semibold text-emerald-700 dark:text-emerald-300"><ShieldCheck className="h-4 w-4" /> Sécurité du compte <ArrowRight className="h-4 w-4" /></Link>
        </div>
        ...
        <section className="mt-6 overflow-hidden rounded-3xl border border-border bg-card shadow-soft">
          <div className="relative h-36 bg-gradient-to-br from-primary via-primary/80 to-accent">
            ...
            >
              {uploading === "banner" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImagePlus className="h-3.5 w-3.5" />} Bannière
            </button>
          ...
          <div className="flex items-end gap-4 px-6 pb-6">
            <div className="relative -mt-12">
              <div className="grid h-24 w-24 place-items-center overflow-hidden rounded-full bg-secondary text-3xl ring-4 ring-card">
                {avatarUrl ? <img src={avatarUrl} alt="" className="h-full w-full object-cover" /> : (form.username[0]?.toUpperCase() ?? "?")}
              </div>
              <button
        ...
        <div className="mt-6 space-y-6">
          <Block title="Identité">
            <Field label="Nom d'utilisateur" error={errors.username}>
              <Input value={form.username} onChange={set("username")} placeholder="globetrotteur" maxLength={30} />
            </Field>
            <Field label="Nom affiché" error={errors.display_name}>
              <Input value={form.display_name} onChange={set("display_name")} placeholder="Camille Martin" maxLength={60} />
            </Field>
            <Field label="Biographie" error={errors.bio}>
              ...
              <span className="mt-1 block text-right text-xs text-muted-foreground">{form.bio.length}/500</span>
            </Field>
          </Block>

          <Block title="Informations personnelles">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Pays" error={errors.country}><Input value={form.country} onChange={set("country")} placeholder="France" /></Field>
              <Field label="Ville" error={errors.city}><Input value={form.city} onChange={set("city")} placeholder="Lyon" /></Field>
              <Field label="Date de naissance" error={errors.birth_date}><Input type="date" value={form.birth_date} onChange={set("birth_date")} /></Field>
              <Field label="Style de voyage" error={errors.travel_style}><Input value={form.travel_style} onChange={set("travel_style")} placeholder="Backpack, slow travel…" /></Field>
            </div>
            <Field label="Langues parlées (séparées par des virgules)">
              <Input value={form.languages} onChange={set("languages")} placeholder="Français, Anglais, Espagnol" />
            </Field>
            <Field label="Centres d'intérêt (séparés par des virgules)">
              <Input value={form.interests} onChange={set("interests")} placeholder="Randonnée, Street food, Photo" />
            </Field>
            <Field label="Pays visités (séparés par des virgules)">
              <Input value={form.visited_countries} onChange={set("visited_countries")} placeholder="Japon, Pérou, Islande" />
            </Field>
          </Block>

          <Block title="Réseaux sociaux">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Site web" error={errors.website_url}><Input value={form.website_url} onChange={set("website_url")} placeholder="https://monblog.com" /></Field>
              <Field label="Instagram" error={errors.instagram}><Input value={form.instagram} onChange={set("instagram")} placeholder="@pseudo" /></Field>
              <Field label="TikTok" error={errors.tiktok}><Input value={form.tiktok} onChange={set("tiktok")} placeholder="@pseudo" /></Field>
              <Field label="YouTube" error={errors.youtube}><Input value={form.youtube} onChange={set("youtube")} placeholder="@chaine" /></Field>
              <Field label="X (Twitter)" error={errors.x_handle}><Input value={form.x_handle} onChange={set("x_handle")} placeholder="@pseudo" /></Field>
            </div>
          </Block>
        </div>

        <div className="sticky bottom-4 mt-8 flex gap-3">
          <Button onClick={save} disabled={saving} size="lg" className="flex-1 gap-2 shadow-elevated">
