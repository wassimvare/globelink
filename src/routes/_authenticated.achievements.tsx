...
import { LEVELS, BADGES, computeXp, getLevel, evaluateBadges, type UserStats } from "@/lib/gamification";
...
function AchievementsPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [posts, trips, entries, myPostIds] = await Promise.all([
        ...
        supabase.from("trip_entries").select("trip_id, country, lat, lng, position").eq("user_id", user!.id),
      ...
      return { posts: posts.data ?? [], trips: trips.data ?? [], entries: entries.data ?? [], likes };
    },
  });
  ...
  const stats: UserStats = useMemo(() => {
    ...
    entries.forEach((e) => {
      if (!e.trip_id) return;
      const arr = byTrip.get(e.trip_id) ?? []; arr.push(e); byTrip.set(e.trip_id, arr);
    });
    let dist = 0;
    byTrip.forEach((arr) => {
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        <header className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Progresse, débloque des badges, monte de niveau.</p>
          </div>
        </header>
        ...
        <section className="mt-8">
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <span className="text-xs text-muted-foreground">{unlockedCount}/{BADGES.length} débloqués</span>
          </div>
              ...
              return (
                <li key={l.key}
                  className={`relative overflow-hidden rounded-2xl border p-3 text-center shadow-soft transition ${
                    isCurrent ? "border-primary bg-card ring-2 ring-primary/40"
                    : reached ? "border-border bg-card"
                    : "border-dashed border-border bg-muted/30 opacity-70"
                  }`}
                >
                  <div className={`mx-auto mb-1 grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br ${l.gradient} text-lg text-white shadow`}>
                    {l.emoji}
                  </div>
                  <p className="text-sm font-semibold">{l.label}</p>
                  <p className="text-[10px] text-muted-foreground">{l.minXp.toLocaleString("fr-FR")} XP</p>
                </li>
              );
            })}
          </ol>
        ...
        <div className="mt-8 text-center">
          <Link to="/dashboard" className="text-sm text-muted-foreground underline underline-offset-4">Voir mon tableau de bord</Link>
        </div>
      </main>
    </div>
  );
    ...
    <div className={`relative overflow-hidden rounded-3xl border border-border bg-card p-5 shadow-soft`}>
      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          <div className={`grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br ${level.current.gradient} text-2xl text-white shadow-glow`}>
            {level.current.emoji}
          </div>
          ...
          <div className="text-right">
            ...
            <p className="font-display text-xl font-semibold"><span ref={xpRef}>0</span></p>
          </div>
        </div>
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
            <span>{level.current.emoji} {level.current.label}</span>
            {level.next ? (
              <span>
...
function StatsStrip({ stats }: { stats: UserStats }) {
  ...
  return (
    <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">
      {items.map((i) => (
        <div key={i.label} className="rounded-2xl border border-border bg-card p-3 text-center shadow-soft">
        ...
        <div className="relative h-1.5 overflow-hidden rounded-full bg-secondary">
          <div
            className={`h-full rounded-full transition-[width] duration-[1200ms] ease-out ${
              b.unlocked ? "bg-gradient-to-r from-emerald-400 to-teal-500" : "bg-gradient-to-r from-primary/60 to-accent/60"
            }`}
            style={{ width: `${width}%` }}
          />
        </div>
