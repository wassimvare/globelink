...
import { PRODUCT_TYPES, formatPrice, type ProductType } from "@/lib/product-catalog";
  ...
  head: () => ({
    meta: [
      { title: "Marketplace créateurs — GlobeLink" },
      {
        name: "description",
        content:
          "Guides PDF, itinéraires, presets Lightroom, ebooks et accompagnements créés par les voyageurs de GlobeLink.",
      },
    ],
  }),
    ...
    return dbProducts
      .map((p) => ({
        ...
        seller: {
          ...
          avatar: p.seller?.avatar_url ?? null,
        },
      }))
      .filter((p) => {
          ...
          {user && (
            ...
            >
              <Link to="/marketplace/new">
                <Plus className="mr-1 h-4 w-4" /> Vendre un produit
              </Link>
            </Button>
          )}
        </header>
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Recherche par destination, tag, mot-clé…"
              className="pl-9"
            />
          </div>
            ...
            {items.map((p) => {
              ...
              return (
                <Link
                  ...
                  to="/marketplace/$id"
                  params={{ id: p.id }}
                  className="group overflow-hidden rounded-2xl border border-border bg-card shadow-soft transition hover:shadow-glow"
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                    {p.cover_url ? (
                      ...
                      <div className="grid h-full place-items-center bg-secondary text-4xl">
                        {typeMeta?.emoji ?? "🧭"}
                      </div>
                    )}
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-2 flex items-center gap-3 text-xs">
                      ...
                      <span className="ml-auto flex items-center gap-1 text-muted-foreground">
                        <Heart className="h-3 w-3" /> {p.favorites_count}
                      </span>
                    </div>
