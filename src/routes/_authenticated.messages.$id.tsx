...
export const Route = createFileRoute("/_authenticated/messages/$id")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Discussion privée GlobeLink avec messages, médias, vocaux et statut vu.",
      },
      { property: "og:title", content: "Conversation — GlobeLink" },
      {
        property: "og:description",
        content: "Messagerie privée sécurisée pour voyageurs GlobeLink.",
      },
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("messages")
        .select(
          "id, content, sender_id, created_at, attachment_url, attachment_type, attachment_meta",
        )
        .eq("conversation_id", id)
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col px-4 py-6">
        <header className="mb-4 flex items-center gap-3">
          ...
          {other?.profile && (
            <>
              <Link
                to="/profile/$username"
                params={{ username: other.profile.username }}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate text-xs text-muted-foreground">
                    {otherTyping ? "en train d'écrire…" : `@${other.profile.username}`}
                  </div>
                </div>
              </Link>
                ...
                <button
                  ...
                  onClick={() =>
                    void startCall({
                      ...
                      recipientAvatar: other.profile?.avatar_url,
                      kind: "audio",
                    })
                  }
        ...
        <div
          ref={scroller}
          className="flex-1 space-y-2 overflow-y-auto rounded-3xl border border-border bg-card p-4 shadow-soft"
        >
          {(!messages || messages.length === 0) && (
            <p className="py-12 text-center text-sm text-muted-foreground">
              Aucun message — dis bonjour ✈️
            </p>
          )}
          {messages?.map((m) => {
            const mine = m.sender_id === user?.id;
            const seen =
              mine && lastOwn?.id === m.id && otherReadAt && new Date(m.created_at) <= otherReadAt;
            return (
              <div key={m.id} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
                <div
                  className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm shadow-soft ${mine ? "gradient-hero text-primary-foreground" : "bg-secondary"}`}
                >
        ...
        <form onSubmit={sendText} className="mt-3 flex items-center gap-2">
          ...
          <button
            ...
            disabled={uploading}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border text-muted-foreground transition hover:bg-secondary"
            aria-label="Photo"
          >
          ...
          <button
            ...
            className={`grid h-10 w-10 shrink-0 place-items-center rounded-full border transition ${recording ? "border-destructive bg-destructive/10 text-destructive animate-pulse" : "border-border text-muted-foreground hover:bg-secondary"}`}
          ...
          <Input
            ...
            placeholder={recording ? "Enregistrement…" : "Écris un message…"}
          ...
          <Button
            ...
            className="rounded-full gradient-hero text-primary-foreground shrink-0"
          >
...
function AttachmentView({ msg }: { msg: Msg }) {
  ...
  if (msg.attachment_type === "share" && meta) {
    return (
      <a
        href={meta.href ?? "#"}
        className="flex w-64 items-center gap-3 rounded-xl bg-background/60 p-2 text-foreground shadow-soft"
      >
        {meta.image ? (
          <img src={meta.image} className="h-14 w-14 rounded-lg object-cover" alt="" />
        ) : (
