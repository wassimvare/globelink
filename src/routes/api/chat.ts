...
type Bucket = { minuteCount: number; minuteReset: number; hourCount: number; hourReset: number; lastSeen: number };
...
async function rateLimit(request: Request) {
  ...
  if (now >= bucket.hourReset) { bucket.hourCount = 0; bucket.hourReset = now + 60 * 60_000; }
  ...
  return {
    ...
    retryAfter: bucket.minuteCount > 10 ? Math.ceil((bucket.minuteReset - now) / 1000) : Math.ceil((bucket.hourReset - now) / 1000),
  };
}
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (origin && origin !== requestUrl.origin) return plainResponse("Origine non autorisée", 403);
        if (fetchSite && !["same-origin", "same-site", "none"].includes(fetchSite)) return plainResponse("Requête non autorisée", 403);

        const limit = await rateLimit(request);
        if (limit.limited) return plainResponse("Trop de requêtes. Réessaie plus tard.", 429, { "Retry-After": String(Math.max(1, limit.retryAfter)) });

        const contentType = request.headers.get("content-type") ?? "";
        if (!contentType.toLowerCase().startsWith("application/json")) return plainResponse("Format non pris en charge", 415);
        const declaredLength = Number(request.headers.get("content-length") ?? 0);
        if (Number.isFinite(declaredLength) && declaredLength > 70_000) return plainResponse("Requête trop volumineuse", 413);

        const rawText = await request.text();
        if (rawText.length > 70_000) return plainResponse("Conversation trop volumineuse", 413);
        const raw = (() => { try { return JSON.parse(rawText); } catch { return null; } })();
        const parsed = bodySchema.safeParse(raw);
        if (!parsed.success) return plainResponse("Conversation invalide", 400);

        const messages = parsed.data.messages as UIMessage[];
        if (messages[messages.length - 1]?.role !== "user") return plainResponse("Le dernier message doit provenir de l'utilisateur", 400);
        const totalText = parsed.data.messages.reduce((total, message) => total + message.parts.reduce((sum, part) => sum + part.text.length, 0), 0);
        if (totalText > 28_000) return plainResponse("Conversation trop longue", 413);

        let ai;
        try { ai = createTravelAiProvider(); }
        catch { return plainResponse("Service temporairement indisponible", 503); }
      ...
      OPTIONS: async ({ request }) => {
        const origin = request.headers.get("origin");
        if (origin && origin !== new URL(request.url).origin) return plainResponse("Origine non autorisée", 403);
        return new Response(null, { status: 204, headers: secureHeaders({ Allow: "POST, OPTIONS" }) });
      },
    },
  },
});
