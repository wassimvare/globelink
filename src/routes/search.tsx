...
import { Search as SearchIcon, X, Loader2, ArrowUpRight, MapPin } from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "Recherche — GlobeLink" },
      {
        name: "description",
        content:
          "Recherche universelle : voyageurs, destinations, activités, hôtels, restaurants, carnets et publications.",
      },
    ],
  }),
...
function SearchPage() {
  ...
  const [activeKind, setActiveKind] = useState<"all" | SearchKind>("all");
  ...
  const total = useMemo(
    () => (data ? Object.values(data).reduce((n, arr) => n + arr.length, 0) : 0),
    [data],
  );
  const hasQuery = debounced.trim().length >= 2;

  const { data: liveSuggestions = [], isLoading: liveSuggestionsLoading } = useQuery({
    queryKey: ["live-catalog", "search-suggestions"],
    queryFn: () => fetchLiveCatalog({ kinds: ["activity", "restaurant", "hotel"], limit: 18 }),
    staleTime: 30 * 60_000,
  });
                ...
                <div className="flex flex-wrap gap-2">
                  {recent.map((r) => (
                    <button
                      key={r}
                      onClick={() => setQ(r)}
                      className="rounded-full border border-border/60 bg-secondary/60 px-3 py-1.5 text-sm hover-scale"
                    >
                      {r}
                    </button>
                  ))}
              ...
              <div className="flex flex-wrap gap-2">
                {suggestions.map((s) => (
                  <button
                    key={s}
                    onClick={() => setQ(s)}
                    className="rounded-full bg-gradient-to-br from-primary/10 to-accent/10 px-3 py-1.5 text-sm font-medium hover-scale"
                  >
                    🔥 {s}
                  </button>
                ))}
            ...
            <section className="mt-8">
              <div className="mb-3 flex items-end justify-between gap-3">
                <div>
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                    Lieux réels à découvrir
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Restaurants, hôtels et activités chargés depuis OpenStreetMap.
                  </p>
                </div>
              ...
              {liveSuggestionsLoading ? (
                ...
                <div className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
                  Le service cartographique est temporairement occupé. Lance une recherche par
                  ville.
                </div>
              )}
            </section>
          ...
          <>
            <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
              <button
                ...
                className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold ${activeKind === "all" ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground"}`}
              >
                Tout
              </button>
              {(Object.keys(KIND_META) as SearchKind[]).map((kind) => {
                const count = data?.[kind]?.length ?? 0;
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold ${activeKind === kind ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground"}`}
                  >
                    {KIND_META[kind].emoji} {KIND_META[kind].label} · {count}
                  </button>
                );
              })}
            </div>
...
function SearchResultThumbnail({
  ...
  const { data: resolvedUrl } = useQuery({
    queryKey: ["search-result-thumbnail", result.image],
    queryFn: () => getSignedMediaUrl(result.image),
    enabled: !!result.image && !result.catalogItem,
    staleTime: 60 * 60_000,
  });
    ...
    return (
      <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-secondary">
        <CatalogImage
          ...
          lookup={{
            latitude: result.catalogItem.latitude,
            longitude: result.catalogItem.longitude,
            city: result.catalogItem.city,
            country: result.catalogItem.country,
            website: catalogOfficialWebsite(result.catalogItem),
          }}
          fallbackIndex={index}
          showIllustrationBadge={false}
  ...
  return (
    <div className="grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 text-xl">
      {emoji}
    </div>
  );
}
