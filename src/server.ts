...
function withSecurityHeaders(response: Response, requestUrl?: URL): Response {
  ...
  const contentSecurityPolicy = [
    ...
    // Vite uses a same-host WebSocket during local mobile testing over HTTP.
    isHttps ? "connect-src 'self' https: wss:" : "connect-src 'self' http: https: ws: wss:",
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    ...
    if (["TRACE", "TRACK", "CONNECT"].includes(method)) {
      return withSecurityHeaders(new Response("Méthode non autorisée", { status: 405 }), requestUrl);
    }
    if (request.url.length > 8_192) {
      return withSecurityHeaders(new Response("URL trop longue", { status: 414 }), requestUrl);
    }
    const declaredLength = Number(request.headers.get("content-length") ?? 0);
    if (requestUrl.pathname.startsWith("/api/") && Number.isFinite(declaredLength) && declaredLength > 1_000_000) {
      return withSecurityHeaders(new Response("Requête trop volumineuse", { status: 413 }), requestUrl);
    }
