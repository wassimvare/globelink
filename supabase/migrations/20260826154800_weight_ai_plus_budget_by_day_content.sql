...
begin
  ...
  -- Keep the category totals chosen by IA+, but distribute each category according
  -- to what is actually planned on each day. This avoids arbitrary 50/50 splits.
  for v_item in
  ...
  loop
    ...
    for v_i in 1..v_date_count loop
      ...
      select lower(concat_ws(' ',
        coalesce((
          select concat_ws(' ', d.headline, d.notes)
            from public.trip_days d
           where d.trip_id = v_trip.id
             and d.user_id = v_trip.user_id
             and d.day_date = v_day
           limit 1
        ), ''),
        coalesce((
          select string_agg(
           ...
           where e.trip_id = v_trip.id
             and e.user_id = v_trip.user_id
             and e.visited_on = v_day
        ), '')
      )) into v_day_text;

      v_day_text := coalesce(v_day_text, '');
      v_score := 1;
      ...
      if v_item.label ~* '(restauration|restaurant|repas|food)' then
        ...
        -- "Divers" / "Extras" represent activities and purchases when IA+ did not
        -- create a more precise category.
        v_score := v_score
        ...
        -- Free activities still count for the itinerary, but should attract less budget.
        if v_day_text ~* '(gratuit|accès libre|acces libre)' then
          v_score := greatest(1, v_score * 0.65);
        end if;
      end if;
    ...
    v_output_label := case
      when v_item.label ~* '^IA\+ · (Divers|Extras)$' then 'IA+ · Activités & achats'
      else v_item.label
    end;
    ...
    for v_i in 1..v_date_count loop
      ...
      if v_piece_cents > 0 then
        insert into public.trip_expenses (
          trip_id, user_id, label, amount, currency, category, spent_on
        ) values (
          v_trip.id,
          v_trip.user_id,
          v_output_label,
...
-- Rebuild existing IA+ forecasts using the itinerary-aware weighting immediately.
do $$
declare
  r record;
begin
