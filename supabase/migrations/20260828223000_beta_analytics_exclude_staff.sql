-- Keep beta analytics focused on external testers, not admin/moderator product usage.
-- Also normalize historical detail routes before they reach the dashboard.
...
begin
  ...
  with beta_users as (
    ...
    where t.user_id is not null
      ...
      and not exists (
        ...
        where ur.user_id = t.user_id and ur.role::text in ('admin', 'moderator')
      )
  ), beta_events as (
    select e.*
    ...
    where e.occurred_at >= v_since
      ...
      and (
        ...
        or not exists (
          select 1 from public.user_roles ur
          where ur.user_id = e.user_id and ur.role::text in ('admin', 'moderator')
        )
      )
  )
  select
  ...
  with beta_users as (
    ...
    select case
      when route like '/profile/%' then '/profile/:username'
      when route like '/trips/%' then '/trips/:id'
      when route like '/post/%' then '/post/:id'
      when route like '/messages/%' then '/messages/:id'
      when route like '/marketplace/%' then '/marketplace/:id'
      when route like '/place-status/%' then '/place-status/:id'
      when route like '/destinations/%' then '/destinations/:slug'
      when route like '/activities/%' then '/activities/:slug'
      when route like '/deals/%' then '/deals/:slug'
      when route like '/questions/%' then '/questions/:slug'
      else route
    end as route
    from beta_events
