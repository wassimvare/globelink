-- Keep IA+ weighted forecasts readable in the carnet: category allocations are
-- rounded to whole euros when the source category total itself is a whole euro.
create or replace function public.globelink_ai_plus_weighted_piece(
...
as $$
  select case
    when p_total_score is null or p_total_score <= 0 then 0
    when p_total_cents % 100 = 0
      then greatest(0, round((p_total_cents::numeric / 100) * p_score / p_total_score)::bigint * 100)
    else greatest(0, floor(p_total_cents::numeric * p_score / p_total_score)::bigint)
  end;
$$;

revoke all on function public.globelink_ai_plus_weighted_piece(bigint, numeric, numeric) from public, anon, authenticated;

-- Patch the current itinerary-aware allocator to use readable amounts.
  ...
  loop
    ...
    for v_i in 1..v_date_count loop
      if v_i=v_date_count then
        ...
        v_piece_cents := least(
          public.globelink_ai_plus_weighted_piece(v_total_cents,v_scores[v_i],v_total_score),
          v_remaining_cents
        );
      end if;
